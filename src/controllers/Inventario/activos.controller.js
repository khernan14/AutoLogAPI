...
export const getActivoById = async (req, res) => {
    try {
        ...
        const [activos] = await pool.query("SELECT * FROM activos WHERE id = ?", [id]);
...
export const createActivo = async (req, res) => {
    ...
    try {
        const { codigo, nombre, modelo = null, serial_number = null, tipo = "Otro", estatus = "Activo", id_bodega, usuario_responsable = null } = req.body;

        if (!id_bodega) {
            connection.release();
            return res.status(400).json({ message: "Debe especificar la bodega inicial" });
        }

        await connection.beginTransaction();

        // evitar duplicado de codigo
        const [dup] = await connection.query("SELECT id FROM activos WHERE codigo = ? LIMIT 1", [codigo]);
        if (dup.length > 0) {
            connection.release();
            return res.status(409).json({ message: "Ya existe un activo con ese 'codigo'." });
        }
       ...
       VALUES (?, 'Bodega', ?, NOW(), 'Ingreso inicial', ?)`,
        ...
        const [rows] = await connection.query("SELECT * FROM activos WHERE id = ?", [activoId]);
...
export const updateActivo = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const { codigo, nombre, modelo = null, serial_number = null, tipo = "Otro", estatus = "Activo" } = req.body;

        const [exists] = await connection.query("SELECT id FROM activos WHERE id = ?", [id]);
        if (exists.length === 0) {
            connection.release();
            return res.status(404).json({ message: "Activo no encontrado" });
        }

        const [dup] = await connection.query("SELECT id FROM activos WHERE codigo = ? AND id <> ? LIMIT 1", [codigo, id]);
        if (dup.length > 0) {
            connection.release();
            return res.status(409).json({ message: "Ya existe un activo con ese 'codigo'." });
        }
       ...
       WHERE id=?`,
        ...
        const [rows] = await connection.query("SELECT * FROM activos WHERE id = ?", [id]);
        res.json(rows[0]);
...
export const getActivosGlobal = async (_req, res) => {
    try {
      ...
      SELECT 
        ...
        b.nombre  AS bodega_nombre
      FROM activos a
      LEFT JOIN ubicaciones_activos ua ON ua.id_activo = a.id AND ua.fecha_fin IS NULL
      LEFT JOIN clientes_sites cs ON ua.id_cliente_site = cs.id
      LEFT JOIN clientes c ON cs.id_cliente = c.id
      LEFT JOIN bodegas b ON ua.id_bodega = b.id
      ORDER BY a.fecha_registro DESC
