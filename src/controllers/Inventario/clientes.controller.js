...
async function saveImage(file, conn) {
    if (!file) return null;
    const baseUrl = process.env.HOST?.replace(/\/$/, "") || "http://localhost:3000";
    const url = `${baseUrl}/uploads/clientes/${file.filename}`;
...
export const createCliente = async (req, res) => {
    ...
    try {
        ...
        if (error.code === "ER_DUP_ENTRY") {
            return res.status(409).json({ message: "El código ya existe, ingrese uno diferente." });
        }
        res.status(500).json({ error: error.message });
    }
    finally {
...
export const updateCliente = async (req, res) => {
    ...
    try {
       ...
       WHERE id=?`,
            [codigo, nombre, descripcion || null, estatus || "Activo", logo_image_id, id]
        );
